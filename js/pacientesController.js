let currentPage = 1;
const recordsPerPage = 2; // Puedes ajustar este valor según tus necesidades
...
async function buscarPacientesHandler(event) {
    event.preventDefault();
    ...
    fetch(`ws_pacientes.php?filtro=${encodeURIComponent(filtro)}`)
        .then(response => response.json())
        .then(data => {
            actualizarListaPacientes(data);
        })
        .catch(error => {
